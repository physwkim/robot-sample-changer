...
복귀합니다. 프로브는 그립을 문 채로(clamped) 합니다 — loosen은 유격
0.05 mm인 스테이지 보어(모드 5)용이고, 유격이 mm급인 홀더 웰에서는
느슨한 핑거가 중력으로만 얹힌 퍽을 웰 밖으로 끌어냅니다(2026-08-19
holder 2에서 실측). 프로브가 전부 성공하면 시트 레벨의 브래킷 중심(base x/y)을
