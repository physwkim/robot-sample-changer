...
**스테이지에는 깊이 널이 없습니다**(`Seat::nullable`이 그 축을 끕니다).
힘만 보면 자세 오차와 구분이 안 갑니다 — 2.05 N에서 물러날수록 1.93,
1.53, 0.92 N으로 곱게 떨어집니다. 그런데 기울기가 37 → 19 → 8 →
2.5 N/mm로 같이 무너지고, −0.93 mm에서 닫힘이 3.9 mm가 아니라
**7.5 mm**에 멈춥니다(넥을 놓치고 더 넓은 면을 잡음, 렌치는 3.4 N으로
튐). 힘이 준 건 시트를 찾아서가 아니라 파지가 풀려서이고, 퍽을 실제로
쥐고 있는 구간 안에는 0이 없습니다. 스테이지 깊이는 닫힘이 퍽을
표면에 눌러 생기는 예압입니다 — 같은 닫힘이 랙 웰에서는 0.13 N입니다.
