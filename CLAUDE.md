...
`Robot:Stop=1`로 멈춰 있는 동안도 이 줄이 말합니다
(`paused by Robot:Stop before step 9`). 그 정지는 `State`가 답할 수
없습니다 — 명령을 읽지 않으므로 Running이어야 하고(서 있는 상태로
찍으면 아무도 안 읽는 GUI 컨트롤이 열립니다), `Alive`도 같이 멈춥니다.
이유를 말할 수 있는 자리가 여기뿐입니다. 예전에는 직전 스텝 이름이
그대로 남아, 다시는 안 움직일 팔 앞에서 화면이 "step 9"라고 했습니다.
...
창은 시트 자세를 따라가지만, **퍽 윗면이 그 자세 어디에 보이는지는 시트마다
다릅니다**. 창의 −6…+2 mm는 랙에서 잰 값이고, 2026-09-03 스테이지를 옮겨
트림으로 다시 티칭한 뒤로는 스테이지 퍽 윗면이 창 맨 아랫줄에 걸쳐
중앙값이 50 mm 뒤 배경을 읽었습니다. 그래서 시트별 **창 바이어스**를
둡니다(`seat_check.rack_window_bias_mm` / `stage_window_bias_mm`, 시트의
툴 프레임 mm). 옮기는 것은 **샘플링 창뿐**이고 판정 기준인 파지점도,
팔이 가는 자세도 그대로입니다 — 트림은 팔이 가는 곳이라 창을 맞추려고
건드리면 안 됩니다. 앉은 퍽으로 실측: +0 → +53.1(비움), +2 → +43.2(비움),
+4 → −8.2, +6 → −7.9, +8 → −7.3, +10 → −7.3. 가장자리가 +3이고 플래토가
+10 너머까지 가므로 스테이지는 **+8 mm**(유효 픽셀 74%), 랙은 0입니다.
`seat_scan <cfg> stage 1 bias:0,8,0`이 라이브 프레임으로 다시 잽니다.
...
**퍽을 든 채 멈췄으면** — 스테이지가 차서 스텝 8 앞에서, 또는 원래
홀더가 차서 스텝 19 앞에서 — 다른 홀더에 넣는 것이 답입니다.
`Holder`를 그 홀더로 두고 `StartStep=18`: 스텝 18이 거기 standby까지
계획해서 가고 19-23이 놓습니다. 집는 다리는 전부 건너뛰므로 손에 든
퍽을 놓치지 않습니다. GUI Sample 카드의 **"Put carried puck in
...
`Hands::before_step(mode, step)`이 **스텝 번호마다** 핑거에 무엇이
있어야 하는지를 답하고, `step_prologue`가 스텝을 실행하기 직전에
그 답과 실제 폭을 맞춰 봅니다. 어긋나면 그 자리에서 정지 — 다른
스텝 실패와 같습니다(팔 정지, 그리퍼 그대로, `CurrentStep` 유지).

- `Free`(열림) — 시트로 **들어가는** 길과 릴리즈 **뒤의** 후퇴:
  스텝 0-4, 11-15, 22-23. 캘리브레이션 hold는 0-4와 22-23.
- `Full`(퍽) — 집은 닫힘부터 놓는 열기까지: 스텝 5-10, 16-21.
  캘리브레이션 hold는 5-21(hold가 그 사이에 서 있으므로).

밴드는 **다리 끝이 아니라 릴리즈에서 끊깁니다**. 11-12·22-23은 놓은
뒤의 후퇴라 손이 비어 있고, 거기가 바로 보호정지가 `CurrentStep`으로
가리키는 스텝입니다.

런 시작에서 한 번만 보지 않는 이유는 **데몬이 서 있는 모든 대기가
`Robot:Gripper`를 서비스**하기 때문입니다(측정 대기, 캘리브레이션
hold, PauseStep 정지). 런 도중에 그리퍼를 열면 팔이 선 자리에 퍽이
떨어지고, 남은 스텝은 빈 손을 시트에 넣고 그 시트를 "참"으로
기록합니다. 반대로 닫아 두면 **닫힌 폭 그대로 시트로 내려갑니다** —
퍽이 있다고 방금 확인한 그 시트로. 운반 중에 퍽이 빠지는 것도 같은
모양이고 누른 사람조차 없습니다. 폭 읽기는 Hand-E 드라이버의 통신
스레드가 계속 갱신하므로 스텝당 비용은 그 스냅샷 한 번입니다.

열기도 **자기 결과를 확인합니다**(`hand`). 예전에는 닫기만
확인했는데(`dead_close`/`empty_close`), 활성화를 잃은 Hand-E는 위치
질의에는 답하고 모션 명령만 무시하므로 스텝 0의 열기가 조용히
아무것도 안 하고 닫힌 핑거가 그대로 시트로 내려갔습니다. 스텝
10·21의 릴리즈에서는 퍽이 잡힌 채로 남아 후퇴가 시트에서 끌어냅니다.

폭 셋은 열림 25 mm, 문 퍽 3.9 mm, 빈 닫힘 0.7 mm이고 경계는
`min_grip_position`(2 mm)과 `open_position - reach_tolerance`
(23.5 mm)입니다. 런이 **시작하기 전에** 걸린 거절 — 퍽을 든 채
멈춘 자리에서 Mount(`StartStep=0`)를 누른 경우 — 은 `Robot:Status`에
이유가 남고 아무것도 움직이지 않으며 `CurrentStep`·`StartStep`도
그대로입니다. 시뮬레이션 그리퍼는 명령한
폭에 정확히 서므로 이 검사를 전혀 하지 않습니다(URSim 리허설은
예전대로).
...
같은 규칙입니다. `Holder=0`은 그립 널 전용이고, 다른 모드는
`compute_run_waypoints`가 거부합니다(랙 피치를 0번째로 외삽해서
홀더 1보다 한 칸 앞으로 가는 일이 없도록).

퍽은 `MapSource`가 정합니다 — 0이거나 대상과 같으면 그 시트에 이미
앉아 있는 퍽을 쓰고, 다른 1-10이면 **먼저 그 홀더에서 가져옵니다**
(모드 7과 같은 이동, 같은 코드). 대상은 두 시트 다 됩니다: 스테이지도
가져오기로 채워지므로 스테이지를 널하려고 Normal 런을 먼저 돌릴
필요가 없습니다. 놓는 다리는 Normal 런 것 그대로라 스텝 번호도
같습니다 — 스테이지는 7-12, 랙은 18-23. 퍽 하나로 랙 전체를 돌 때
트리거 한 번에 가져오기+널이 끝나라고 있는 옵션입니다. 널이 끝나면
퍽은 **대상 시트에 남습니다** — 다음 홀더의 소스가 바로 여기이고,
스테이지에 널했다면 거기 놓인 채이므로 되돌리는 건 Return입니다.
소스는 항상 랙 홀더입니다 — `MapSource=0`은 "제자리 퍽"이 이미 쓰고
있어서 스테이지를 가리킬 번호가 없습니다. 소스를 지정할 때 대상
시트가 비었는지는 `seat_check`가 봅니다(모드 7과 같은 경로) — 꺼져
있으면 아무도 안 봅니다.
