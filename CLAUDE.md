...
ros2 launch ur3e_hande_robot_description ur_control.launch.py \
  ur_type:=ur3e robot_ip:=192.168.5.10 \
  ...
  gripper_use_fake_hardware:=false use_tool_communication:=true \
  tool_voltage:=24 launch_rviz:=false

# 2. MoveIt
ros2 launch ur_moveit_config ur_moveit.launch.py \
  ur_type:=ur3e description_package:=ur3e_hande_robot_description \
  description_file:=ur_with_hande.xacro \
  moveit_config_package:=ur3e_hande_moveit_config \
  moveit_config_file:=ur.srdf

# 3. EPICS Sequence
ros2 run epics_robot epics_triggered_sequence --ros-args \
...
# 4. Stage Scene
ros2 run stage_scene_utils add_stage_to_scene --ros-args \
