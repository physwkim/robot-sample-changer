...
복귀합니다. 프로브는 그립을 문 채로(clamped) 합니다 — loosen은
스테이지 보어(모드 5)용이고, 홀더 웰도 편측 0.05 mm로 똑같이
타이트하지만(holder 4 실측) 퍽을 중력으로만 잡아서 느슨한 핑거가 넥을
밀면 퍽이 기울어 웰 밖으로 빠집니다(holder 2 실측, 둘 다 2026-08-19).
한계: 티칭 잔차가 유격을 넘으면 프로브가 프리로드로 시작해 포화하므로
(첫 0.05 mm 스텝에 6.9 N), 브래킷이 잡는 잔차는 유격 이내 =
deadband 이하라 현재 수치로는 trim 쓰기가 일어나지 않습니다. 프로브가 전부 성공하면 시트 레벨의 브래킷 중심(base x/y)을
그 홀더의 x/z 트림에 **직접 반영**합니다 — `taught_waypoints.yaml`을
텍스트 편집(주석 보존, tmp+rename, 재파싱 검증)으로 갱신하고 다음
트리거가 그대로 다시 읽습니다. 중심이 lateral step(0.05 mm) 미만인
축은 그대로 두고, 1 mm 초과면 시트 이상으로 보고 쓰기를 거부합니다. 스텝 번호와 웨이포인트는 Normal 시퀀스 그대로라
