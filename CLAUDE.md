...
| Robot:MapSource | longout | 홀더 간 이동(모드 7)의 소스 홀더 (1-10) |
...
주지 않는 위치**로 몰아넣습니다. 반복마다 제자리 퍽을 집고(스텝 0-5)
바로 되놓으며(20-23), 닫는 순간 툴에 남은 렌치를 읽어 그만큼 트림을
...
판정에 넣지도 않습니다 — 한 임계값이 두 일을 합니다. 루프가 닫혀
있으므로 `stiffness_n_per_mm`은 **수렴 속도만 정하고 답은 정하지
않습니다**; 두 홀더 실측값이라 `damping` 0.7로 2배 오차까지 수축을
보장합니다. 반복당 0.5 mm, 누적 1 mm를 넘는 요구는 트림 오차가
아니라 시트 이상으로 보고 거부합니다.
...
이미 찬 시트로의 릴리즈가 되므로 거부). **홀더에 퍽이 있어야
합니다** — 빈 시트는 `empty_close`가 잡아 중단합니다. 실패해도 퍽은
시트에, 팔은 스탠바이에 있고 데몬은 트리거 대기로 돌아갑니다.
